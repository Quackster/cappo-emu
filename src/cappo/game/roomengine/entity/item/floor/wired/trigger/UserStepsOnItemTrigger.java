...
public class UserStepsOnItemTrigger
  extends WiredTriggerBase
{
  ...
  public void setupItem(FloorItem item)
  {
    ...
    {
      List<WiredTriggerBase> triggerList = (List)this.wiredManager.triggersWalksOnFurni.get(Integer.valueOf(Tile.xy));
      if (triggerList == null)
      {
        triggerList = new ArrayList();
        this.wiredManager.triggersWalksOnFurni.put(Integer.valueOf(Tile.xy), triggerList);
      }
      if (triggerList.isEmpty()) {
        item.eventSetFlag(Tile.xy, 256, true);
      }
  ...
  private void removeItem(FloorItem item, int xy, Direction8 dir)
  {
    ...
    for (RoomFloorItemData.AffectedTile Tile : PointList)
    {
      ...
      if (triggerList != null)
      {
        ...
        if (triggerList.isEmpty())
        {
          this.wiredManager.triggersWalksOnFurni.remove(Integer.valueOf(Tile.xy));
          item.eventSetFlag(Tile.xy, 256, false);
        }
      }
    }
  }
