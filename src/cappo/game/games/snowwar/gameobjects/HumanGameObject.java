...
import cappo.game.games.snowwar.SpawnPoint;
  ...
  extends GameItemObject
{
  ...
  public static final int[] boundingData = { 1600 };
  ...
  public HumanGameObject(SnowWarRoom room, int teamId)
  {
    ...
    if (teamId == 1)
    {
      int i = Utils.GetRandomNumber(0, this.currentSnowWar.ArenaType.spawnsBLUE.size() - 1);
      spawn = (SpawnPoint)this.currentSnowWar.ArenaType.spawnsBLUE.get(i);
    }
    else
    {
      int i = Utils.GetRandomNumber(0, this.currentSnowWar.ArenaType.spawnsRED.size() - 1);
      spawn = (SpawnPoint)this.currentSnowWar.ArenaType.spawnsRED.get(i);
    }
  ...
  public void subturn(SynchronizedGameStage unused)
  {
    ...
    if (this.fireRateLimiter > 0) {
      this.fireRateLimiter -= 1;
    }
    if (this.pickUpLimiter > 0) {
      this.pickUpLimiter -= 1;
    }
    if ((canWalk()) && (this.currentTile != null)) {
      ...
      {
        if ((this.pickUpLimiter < 1) && 
          (this.currentTile.pickBallsItem != null) && (this.currentTile.pickBallsItem.canPickUpFromHere()))
        {
          this.currentTile.pickBallsItem.concurrentUses += 1;
        ...
        if (!this.currentTile.isTooAway(this.moveTarget))
        {
          ...
          {
            if ((this.nextTile != null) && (!this.nextTile.isOpen(this)) && 
              (this.moveTarget.isSamePosition(this.nextTile.location())))
            {
              setNextTile(null);
              stopWalk();
              return;
  ...
  private void doFire()
  {
    ...
    if (items.size() == 1)
    {
      GamefuseObject item = (GamefuseObject)items.get(0);
  ...
  {
    if ((this.team != _arg2.team) || (SnowWar.TEAMS.length == 1))
    {
      this.hits += 1;
      giveScore(1);
    }
  ...
  public void giveScore(int _arg2)
  {
    ...
    this.currentSnowWar.TeamScore[(this.team - 1)] += _arg2;
  }
